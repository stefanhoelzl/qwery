...
>
  ...
  >
    <button
      type="button"
      class="focus:outline-hidden items-center justify-center rounded-lg border border-transparent bg-transparent text-sm font-medium text-gray-500 disabled:pointer-events-none disabled:opacity-50 dark:text-gray-400"
      aria-haspopup="menu"
      aria-expanded={open}
      onclick={() => (open = !open)}
    >
  ...
  <div
    class="z-1 absolute mt-2 max-h-80 min-w-80 overflow-scroll rounded-lg bg-white shadow-md transition-[opacity,margin] dark:divide-neutral-700 dark:border dark:border-neutral-700 dark:bg-neutral-800"
    class:hidden={!open}
    role="menu"
  >
    {@render children()}
